...
def main():
    ...
    if args.domain:
        ...
        commands = [
            ...
            ("", lambda domain, output_dir: execute_command("cat finalsubs.txt | dnsx -silent | httpx | tee -a filterDNS.txt")),
            ("", lambda domain, output_dir: os.rename("filterDNS.txt", os.path.join(output_dir, "filterDNS.txt"))),
            ("", lambda domain, output_dir: execute_command(f"sed 's/https:\/\///' output/filterDNS.txt | tee -a output/nonhttpsfilterDNS.txt")),
            ...
            ("", lambda domain, output_dir: execute_command(f"paramspider -l output/filterDNS.txt | tee -a output/Params_list.txt ; mkdir output/Paramspider_Result ; mv results output/Paramspider_Result")),
            ("SubDomain Takeover", lambda domain, output_dir: execute_command(f"python3 Tools/sub404/sub404.py -f output/filterDNS.txt  | tee -a output/Sub404result.txt")),
            ("", lambda domain, output_dir: execute_command(f"python3 Tools/sub404/sub404.py -d {domain} -p https | tee -a output/Sub404result1.txt")),
            ("", lambda domain, output_dir: execute_command(f"subjack -w output/filterDNS.txt -a -v | tee -a output/subjackresults.txt")),
            ...
            ("SSRF Running", lambda domain, output_dir: execute_command(f"cat output/nonhttpsfilterDNS.txt | gau | ./Tools/bulkssrf/target/release/bssrf -v -t 10 -l  {get_url_from_config(args.config, 'BURP_COLLAB_URL')} | tee -a output/bulkssrf_result.txt")),
            ("", lambda domain, output_dir: execute_command(f"cat output/all_target_urls.txt | ./Tools/ssrfuzz/ssrfuzz scan -x 'GET','POST' | tee -a ssrfuzz_results.txt")),
            ...
            ("X-Ray All_in_one Vulnerability Scanner", lambda domain, output_dir: execute_command(f"cd  Tools/xray/; while read domain; do ./xray_linux_386 ws --basic-crawler  $domain ; done <  ../../output/all_target_urls.txt | tee -a ../../output/XRay_All_in_one_Scanner.txt ;  cd ../../")),
            ("", lambda domain, output_dir: execute_command(f"cd  Tools/xray/; while read domain; do ./xray_linux_386 ws --browser-crawler  $domain ; done <  ../../output/all_target_urls.txt | tee -a ../../output/XRay_All_in_one_Scanner.txt ;  cd ../../")),
